...
| Check | Expected-value source | Where it runs | Status |
|---|---|---|---|
| No hardcoded sensor dimension above the driver layer | External contract — `PHILOSOPHY.md`'s sensor-profile rule | Sandbox or Pi (source-inspection only, no hardware) | Implemented — `camera_backend.py`'s `assert_only_camera_backend_imports_sensor_profiles` (source-scans the whole tree for direct `imx477`-style imports outside `camera_backend.py`). |
