...
| `hdr_merge`'s `merge()` weights saturation by a continuous per-photosite clean-sample fraction (from `frame_average.py`'s own excluded-count sibling), not a binary any-clipped threshold on its own averaged input; a bracket with no such record merges without exclusion, logged | Observed behavior (synthetic masters + excluded-count siblings with known, distinct clean-counts, plus a matched no-sibling case) | Sandbox or Pi (`tifffile`/`numpy` only, no camera) | Implemented — `hdr_merge.py`'s own `render_check()`. Numerically, not just structurally: a 1-of-8-clipped photosite's merged estimate is hand-computed via the exact documented formula at `clean_fraction=0.875` and confirmed to differ from what the WRONG (binary-exclusion, `clean_fraction=0`) answer would have been, then confirmed to match the real output exactly; an 8-of-8-clipped photosite's merged estimate is confirmed to collapse to exactly the other exposure's own irradiance estimate (zero weight, not approximately zero). A separate case with no `_excluded_count.tif` sibling confirms `mask_used=False` structurally (`info["exposures"][i]["exclusion"]`) AND that the missing-mask log line prints exactly once per un-masked exposure (captured stdout, not inferred), AND that the resulting weight equals the pure mid-tone-hat formula with no exclusion applied, hand-computed the same way. Covers: the clean-fraction arithmetic, the missing-mask code path, and that both are reachable through `merge()`'s own real call path. Does NOT cover: real-bracket behavior against `2026-08-03_050600`'s own actual masks (see `CHANGELOG.md`'s own build-record entry for that verification, run separately against real files, not a self-check); whether a mixed bracket (some exposures with masks, others without) behaves sensibly — every real bracket so far is uniformly masked or unmasked, and no synthetic case exercises the mixed scenario. |
